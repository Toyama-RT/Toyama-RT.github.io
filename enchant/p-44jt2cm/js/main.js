enchant(); // enchant.js を利用したコミュニケーションエイド
//よく見えない事例対策　５０音文字盤を二画面に分けて表示
...
window.onload = function() {
    ...
    var scx = wiw * 3.0 / 1280;// 文字キー幅の係数
    var scy = wih * 2.6 / 1024;// 文字キー高さ係数
    ...
    game_.onload = function() { // メイン処理実行
      ...
      // 画面１
          var create1stScene = function() {
                ...
                //scene.backgroundColor = '#ffffff'; // 背景色設定White
                //scene.backgroundColor  = '#d3d3d3'; // 背景色設定LightGrey
                scene.backgroundColor  = '#e6e6fa'; // 背景色設定Lavender
	...
	//画像の設定
        ...
        var wordyellow = new Sprite(50, 50);
        ...
        wordyellow.image = game_.assets[imgyellow];

	worda.x = wiw * 0.85; // wordaの横位置を設定します。
	...
	wordyellow.x = wiw * 0.96;
        wordyellow.y = wih * 0.6;
	wordyellow.scaleX = scx/2;
	wordyellow.scaleY = scy;
        ...
        scene.addChild(wordyellow);


            //var title = new Label('');         //文字列の表示
            title.color = '#000000';            // 文字色
		if (window.innerWidth > 1000){  //画面の大きさでフォントを大きさを変更する
		title.font = '70px sans-serif';            // 65pxのゴシック体にする
		} else if (window.innerWidth < 500){
		title.font = '30px sans-serif';            // 30pxのゴシック体にする
		} else {
		title.font = '38px sans-serif';            // 38pxのゴシック体にする
		}
            title.textAlign = 'left';           // 左揃え
            title.width = wiw * 0.9;            // 自動改行の指定
            title.x = wiw * 0.03;                // 横位置調整
            title.y = wih * 0.02;               // 縦位置調整
            scene.addChild(title);    // シーンに追加

            scene.addEventListener(Event.ENTER_FRAME, function(e) { //titleの表示不良対策としてフレームイベントに表示機能をつけた。あまりいいやり方ではない
            scene.addChild(title);    // シーンに追加
            });
		...
		wordred.tl.scaleBy( 0.9, 3, enchant.Easing.ELASTIC_EASEINOUT);
            ...
            wordyellow.addEventListener(Event.TOUCH_START, function(e) {
		wordred.tl.scaleBy( 0.9, 3, enchant.Easing.ELASTIC_EASEINOUT);
                wordred.tl.scaleBy( 1 / 0.9, 3, enchant.Easing.ELASTIC_EASEINOUT);
                game_.pushScene(create2ndScene()); // 画面２に切り替え
	...
	wordyellow.scaleY = scy;
        ...
        scene.addChild(wordyellow);
            ...
            title.color = '#000000';            // 文字色
		if (window.innerWidth > 1000){  //画面の大きさでフォントを大きさを変更する
		title.font = '70px sans-serif';            // 37pxのゴシック体にする
		} else if (window.innerWidth < 500){
		title.font = '30px sans-serif';            // 30pxのゴシック体にする
		} else {
...
*/
	...
	wordhai2.x = wiw * 0.12;
        wordhai2.y = wih * 0.3;
	wordhai2.scaleX = scx * 0.6;
	wordhai2.scaleY = scy * 0.65;
	wordiie2.x = wiw * 0.12;
        wordiie2.y = wih * 0.4;
	wordiie2.scaleX = scx * 0.6;
	wordiie2.scaleY = scy * 0.65;
	worddochirademoii.x = wiw * 0.12;
        worddochirademoii.y = wih * 0.5;
	worddochirademoii.scaleX = scx * 0.6;
	worddochirademoii.scaleY = scy * 0.65;
	worddochirademonai.x = wiw * 0.12;
        worddochirademonai.y = wih * 0.6;
	worddochirademonai.scaleX = scx * 0.6;
	worddochirademonai.scaleY = scy * 0.65;

	wordmouichido.x = wiw * 0.43;
        wordmouichido.y = wih * 0.3;
	wordmouichido.scaleX = scx * 0.6;
	wordmouichido.scaleY = scy * 0.65;
	wordmatte.x = wiw * 0.43;
        wordmatte.y = wih * 0.4;
	wordmatte.scaleX = scx * 0.6;
	wordmatte.scaleY = scy * 0.65;
	wordarigatou.x = wiw * 0.43;
        wordarigatou.y = wih * 0.5;
	wordarigatou.scaleX = scx * 0.6;
	wordarigatou.scaleY = scy * 0.65;
	wordonegai.x = wiw * 0.43;
        wordonegai.y = wih * 0.6;
	wordonegai.scaleX = scx * 0.6;
	wordonegai.scaleY = scy * 0.65;

	wordkonnichiwa.x = wiw * 0.74;
        wordkonnichiwa.y = wih * 0.3;
	wordkonnichiwa.scaleX = scx * 0.6;
	wordkonnichiwa.scaleY = scy * 0.65;
	wordsayounara.x = wiw * 0.74;
        wordsayounara.y = wih * 0.4;
	wordsayounara.scaleX = scx * 0.6;
	wordsayounara.scaleY = scy * 0.65;
	wordohayou.x = wiw * 0.74;
        wordohayou.y = wih * 0.5;
	wordohayou.scaleX = scx * 0.6;
	wordohayou.scaleY = scy * 0.65;
	wordoyasumi.x = wiw * 0.74;
        wordoyasumi.y = wih * 0.6;
	wordoyasumi.scaleX = scx * 0.6;
	wordoyasumi.scaleY = scy * 0.65;
	wordkyuuin.x = wiw * 0.74;
        wordkyuuin.y = wih * 0.7;
	wordkyuuin.scaleX = scx * 0.6;
	wordkyuuin.scaleY = scy * 0.65;


	wordzenbukesu.x = wiw * 0.9;
