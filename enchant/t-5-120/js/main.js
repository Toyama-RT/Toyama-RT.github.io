...
*/
            var title0 = new Label('１２０分間タイマー');
		title0.color = '#000000';
		...
		title0.y = 10;
            ...
            var title0e = new Label('120min Timer');
    ...
    game_.preload('arm03.gif', 'arm04.gif', 'arm05.gif', 'sw1.gif', 'dog1.mp3', 'sw2.gif', 'chick-cry1.mp3', 'sw3.gif', 'chicken-cry1.mp3', 'cat-cry1.mp3', 'corvus-macrorhynchos-cry1.mp3', 'jump.mp3', 'gameover.mp3');
// 感謝　効果音ラボ
	...
	arm3.scaleY = scy * 1.2;
        ...
        // 「タッチイベント」
            sw1.addEventListener(Event.TOUCH_START, function(e) {
                game_.assets['dog1.mp3'].clone().play();
		if (Ptime * dps >= 180){
		...
		game_.assets['chick-cry1.mp3'].clone().play();
		Tadd = 0;
            });
		...
		Ptime = Ptime + Tadd;
            		titleT1.text = Ptime;
            		titleM1.text = Math.floor(Ptime / 60);
            		titleS1.text = Ptime % 60;
		if (Ptime * dps <= 360){
		arm1.rotation = Ptime * dps;
		}
		if (Ptime * dps == 90){
		game_.assets['corvus-macrorhynchos-cry1.mp3'].clone().play();
		}
		if (Ptime * dps == 180){
        	game_.rootScene.removeChild(arm2);
        	game_.rootScene.addChild(arm3);
		game_.assets['cat-cry1.mp3'].clone().play();
		}
		if (Ptime * dps == 270){
		game_.assets['corvus-macrorhynchos-cry1.mp3'].clone().play();
		}
		if (Ptime * dps == 360){
		game_.assets['chicken-cry1.mp3'].clone().play();
		}
        });
