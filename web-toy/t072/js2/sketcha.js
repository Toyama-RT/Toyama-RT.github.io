...
//	manager.createCircle(type, canvas.width * 75 / 100, canvas.height * 10 / 100, CirImgb, radiusball, 0, null, null, null);
	...
	function createFrame(){
		...
		// Remover
		let remover1 = manager.createBody(type, canvas.width /2 - div, ch2 /2 , ch2 /1, 10, 90);
		remover1.SetUserData({tag: TAG_REMOVER});
		let remover2 = manager.createBody(type, canvas.width /2 + div, ch2 /2 , ch2 /1, 10, 90);
		remover2.SetUserData({tag: TAG_REMOVER});

		let remover3 = manager.createBody(type, canvas.width /2 , ch2 /2 - div/2, ch2 /1, 10, 0);
		remover3.SetUserData({tag: TAG_REMOVER});
		let remover4 = manager.createBody(type, canvas.width /2 , ch2 /2 + div/2, ch2 /1, 10, 0);
